...
// stringClean("Hello") → "Helo
    ...
    //leetcode (https://leetcode.com/problems/jewels-and-stones/) Q771
    public int numJewelsInStones(String jewels, String stones) {
        if (stones.length() == 0) {
            return 0;
        }
        int count = 0;
        char currSt = stones.charAt(0);

        if (jewels.indexOf(currSt) != -1) {
            count = 1;
        }
...
// countHi2("xhixhi") → 0
    ...
    public String helper(String s) {
        ...
        char nxChar = s.charAt(1);
        if (currChar == nxChar) {
            return helper(s.substring(1)); //skip for 1 
        } else {
            return 
        }
    }
